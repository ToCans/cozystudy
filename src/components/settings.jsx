				...
				<HiOutlineArrowCircleLeft
					...
					onClick={() => {
						if (settings.backgroundImageIndex > 0) {
							...
							settings.setBackgroundImageIndex(settings.backgroundImages.length - 1);
						}
					}}
				/>
				<p className="text-center align-middle select-none w-2/5">
					Background Image:
					{settings.backgroundImageNames[settings.backgroundImageIndex]}
				</p>
				<HiOutlineArrowCircleRight
					className="size-6 sm:mt-1 hover:stroke-slate-600"
					onClick={() => {
						if (settings.backgroundImageIndex < settings.backgroundImages.length - 1) {
							settings.setBackgroundImageIndex(settings.backgroundImageIndex + 1);
						} else {
				...
				<HiOutlineArrowCircleLeft
					...
					onClick={() => {
						if (settings.themeIndex > 0) {
							...
							settings.setThemeIndex(settings.themeNames.length - 1);
						}
					}}
				/>
				<p className="text-center align-middle select-none w-2/5">
					Theme:
					{settings.themeNames[settings.themeIndex]}
				</p>
				<HiOutlineArrowCircleRight
					className="size-6 sm:mt-1 hover:stroke-slate-600"
					onClick={() => {
						if (settings.themeIndex < settings.themeNames.length - 1) {
							settings.setThemeIndex(settings.themeIndex + 1);
						} else {
