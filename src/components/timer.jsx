...
function Timer() {
	...
	const [progressBarTotal, setProgressBarTotal] = useState(settings.workingMinutes * 60);
	...
	useEffect(() => {
		let calculatedProgressBarValue =
			(1 - (minutesRemaining * 60 + secondsRemaining) / progressBarTotal) * 100;
	...
	useEffect(() => {
		if (minutesRemaining === 0 && secondsRemaining === 0 && timerRunning === true) {
	...
	return (
		...
		>
			...
			<div className="row flex flex-row justify-center w-full space-x-2 m-1">
				<SoundButton purpose="None" audioPlayingSetter={settings.setAudioPlaying} />
				<SoundButton purpose="Fire" audioPlayingSetter={settings.setAudioPlaying} />
				<SoundButton purpose="Wind" audioPlayingSetter={settings.setAudioPlaying} />
				<SoundButton purpose="Rain" audioPlayingSetter={settings.setAudioPlaying} />
			</div>
		</div>
	);
}
